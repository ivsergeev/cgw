...
Скилы можно настроить на подтверждение перед выполнением. Когда у скила `confirm: true`, агент должен использовать `cgw_invoke_confirmed` вместо `cgw_invoke`. Клиент агента (напр. OpenCode) покажет нативное подтверждение.
...
Замените `<name>` на ключ из секции `mcp` (напр. `corp`). Секция `permissions` заставляет OpenCode спрашивать подтверждение перед выполнением подтверждаемых скилов.
