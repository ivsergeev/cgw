...
Скилы можно настроить так, чтобы они требовали подтверждения перед выполнением. Это защищает от непреднамеренных действий AI-агента.

### Как это работает

Когда у скила `confirm: true`, агент должен использовать `cgw_invoke_confirmed` вместо `cgw_invoke`. Ответ `cgw_schema` включает флаг `confirm` и рекомендуемое имя инструмента `invoke`.

**Flow:**

1. Агент вызывает `cgw_schema(skill)` → видит `confirm: true, invoke: "cgw_invoke_confirmed"`
2. Агент вызывает `cgw_invoke_confirmed(skill, params)`
3. Клиент (напр. OpenCode) показывает нативное подтверждение
4. Пользователь подтверждает → скил выполняется

Если агент ошибочно вызовет `cgw_invoke` для подтверждаемого скила, расширение блокирует вызов с ошибкой: *"Use cgw_invoke_confirmed"*.
...
В настройках расширения (⚙ Настройки → **Режим подтверждения**):

- **Нативный** (по умолчанию) — инструмент `cgw_invoke_confirmed` доступен, агент использует его для подтверждаемых скилов. Требует настройку `permission` в агенте (напр. OpenCode).
- **OTP** — `cgw_invoke_confirmed` скрыт, подтверждаемые скилы используют 4-значный код через системное уведомление. Работает с любым агентом без настройки.

### Настройка для каждого скила

В редакторе скила используйте чекбокс **«Требовать подтверждение»**:
- **Включён** — скил требует подтверждения перед выполнением
