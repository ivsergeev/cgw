...
### OTP fallback (опционально)

Если ваш агент не поддерживает tool permissions, включите OTP fallback в настройках расширения (⚙ Настройки → **«OTP fallback для подтверждаемых скилов»**). При включении `cgw_invoke` для подтверждаемых скилов запускает код через системное уведомление вместо блокировки.
