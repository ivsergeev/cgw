...
**Основной flow (OpenCode и агенты с системой permissions):**
...
3. OpenCode показывает нативное подтверждение в терминале
...
1. Агент вызывает `cgw_invoke(skill, params)` для скила с `confirm: true`
2. Расширение генерирует 4-значный код, показывает через системное уведомление
3. Расширение возвращает «требуется подтверждение — спросите код у пользователя»
4. Агент спрашивает код, вызывает повторно с `confirmCode`
5. Расширение проверяет код и выполняет
